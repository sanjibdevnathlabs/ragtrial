...
class TestDocumentRetrieverInit:
    ...
    def test_init_success(self, mock_config):
        ...
        with patch(
            "app.chain_rag.retriever.create_embeddings"
        ) as mock_create_emb, patch(
            "app.chain_rag.retriever.create_vectorstore"
        ) as mock_create_vs:
    ...
    def test_retrieve_success(self, mock_config):
        ...
        with patch(
            ...
            mock_vs.query.assert_called_once_with(
                "test query", n_results=constants.DEFAULT_RETRIEVAL_K
            )

    def test_retrieve_empty_query_raises_error(self, mock_config):
        """Test that empty query raises ValueError."""
        with patch("app.chain_rag.retriever.create_embeddings"), patch(
            "app.chain_rag.retriever.create_vectorstore"
        ):
