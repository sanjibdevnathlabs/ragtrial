...
class TestIngestDocuments:
    ...
    def test_ingest_no_unindexed_files(
        self,
        mock_config,
        mock_file_service
    ):
    ...
    def test_ingest_single_file_success(
        ...
        with patch('ingestion.ingest.FileService', return_value=mock_file_service), \
             patch('ingestion.ingest.create_embeddings', return_value=mock_embeddings), \
             patch('ingestion.ingest.create_vectorstore', return_value=mock_vectorstore), \
             patch('ingestion.ingest.DocumentLoader', return_value=mock_loader), \
             patch('ingestion.ingest.DocumentSplitter', return_value=mock_splitter), \
             patch('ingestion.ingest.LoaderFactory.is_supported', return_value=True):
    ...
    def test_ingest_unsupported_format_skipped(
        ...
        with patch('ingestion.ingest.FileService', return_value=mock_file_service), \
             patch('ingestion.ingest.create_embeddings', return_value=mock_embeddings), \
             patch('ingestion.ingest.create_vectorstore', return_value=mock_vectorstore), \
             patch('ingestion.ingest.LoaderFactory.is_supported', return_value=False):
...
class TestMain:
    ...
    def test_main_success(self):
        ...
        with patch('ingestion.ingest.parse_arguments') as mock_parse, \
             patch('ingestion.ingest.Config', return_value=mock_config), \
             patch('ingestion.ingest.setup_logging'), \
             patch('ingestion.ingest.ingest_documents', return_value=(10, 0, 0, 100)), \
             patch('ingestion.ingest.print_summary'):
    ...
    def test_main_config_file_not_found(self):
        ...
        with patch('ingestion.ingest.parse_arguments'), \
             patch('ingestion.ingest.Config', side_effect=FileNotFoundError("Config not found")):
    ...
    def test_main_config_load_error(self):
        ...
        with patch('ingestion.ingest.parse_arguments'), \
             patch('ingestion.ingest.Config', side_effect=Exception("Config error")):
    ...
    def test_main_with_failures(self):
        ...
        with patch('ingestion.ingest.parse_arguments') as mock_parse, \
             patch('ingestion.ingest.Config', return_value=mock_config), \
             patch('ingestion.ingest.setup_logging'), \
             patch('ingestion.ingest.ingest_documents', return_value=(5, 3, 0, 50)), \
             patch('ingestion.ingest.print_summary'):
    ...
    def test_main_no_successful_files(self):
        ...
        with patch('ingestion.ingest.parse_arguments') as mock_parse, \
             patch('ingestion.ingest.Config', return_value=mock_config), \
             patch('ingestion.ingest.setup_logging'), \
             patch('ingestion.ingest.ingest_documents', return_value=(0, 0, 5, 0)), \
             patch('ingestion.ingest.print_summary'):
    ...
    def test_main_keyboard_interrupt(self):
        ...
        with patch('ingestion.ingest.parse_arguments'), \
             patch('ingestion.ingest.Config', side_effect=KeyboardInterrupt()):
    ...
    def test_main_unexpected_exception(self):
        ...
        with patch('ingestion.ingest.parse_arguments'), \
             patch('ingestion.ingest.Config', side_effect=RuntimeError("Unexpected error")):
