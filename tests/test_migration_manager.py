...
class TestEnsureMigrationsTableExists:
    ...
    def test_table_already_exists(self):
        ...
        with patch.object(manager.session_factory, 'get_write_engine', return_value=mock_engine):
...
""")
        ...
        with patch.object(manager.session_factory, 'get_write_engine', return_value=mock_engine), \
            ...
            manager.apply_migration("20250128_000001_create_files_table", migration_file)
...
""")
        ...
        with patch.object(manager.session_factory, 'get_write_engine', return_value=mock_engine):
            with pytest.raises(DatabaseMigrationError):
                manager.apply_migration("20250128_000001_failing_migration", migration_file)
...
""")
        ...
        with patch.object(manager.session_factory, 'get_write_engine', return_value=mock_engine):
            manager.rollback_migration("20250128_000001_create_files_table", migration_file)
    ...
    def test_rollback_migration_no_down_function(self, tmp_path):
        ...
        with patch.object(manager.session_factory, 'get_write_engine', return_value=mock_engine):
            with pytest.raises(DatabaseMigrationError):
                manager.rollback_migration("20250128_000001_bad_migration", migration_file)
...
""")
        ...
        with patch.object(manager.session_factory, 'get_write_engine', return_value=mock_engine):
            with pytest.raises(DatabaseMigrationError):
                manager.rollback_migration("20250128_000001_failing_migration", migration_file)
...
class TestGenerateVersion:
    ...
    def test_generate_version_sanitizes_description(self):
        ...
        with patch('migration.manager.datetime') as mock_datetime:
            ...
            assert version == "20250128_143022_add_user_index"  # Dashes become underscores
            
            version = manager.generate_version("fix  multiple   spaces")
            assert version == "20250128_143022_fix__multiple___spaces"  # Multiple spaces become multiple _
    ...
    def test_generate_version_removes_special_chars(self):
        ...
        with patch('migration.manager.datetime') as mock_datetime:
            ...
            assert version == "20250128_143022_fix__bug"  # Spaces become _, special chars removed
