...
def test_embeddings(config: Config, logger) -> bool:
    ...
    logger.info(
        ...
        message=constants.TESTING_EMBEDDINGS,
    )
    logger.info(
    ...
    try:
        ...
        logger.info(
            ...
            text_length=len(constants.TEST_SENTENCE),
            message=constants.GENERATING_EMBEDDING,
        )
        ...
        if not embedding or len(embedding) == 0:
            logger.error(
                codes.EMBEDDINGS_ERROR, message=constants.EMPTY_EMBEDDING_RETURNED
            )
            return False
        ...
        logger.warning(
            codes.TEST_FAILED, test_type="embeddings", reason="user_interrupted"
        )
        raise
...
def _add_test_document(vectorstore, logger) -> None:
    ...
    test_metadata = {
        constants.TEST_DOCUMENT_SOURCE: "setup_test",
        "type": constants.TEST_DOCUMENT_TYPE,
    }
    ...
    logger.info(
        ...
        count=1,
        message=constants.TEST_DOCUMENT_ADDED,
    )
...
def test_vectorstore(config: Config, embeddings, logger) -> bool:
    ...
    logger.info(
        codes.TEST_STARTING,
        test_type="vectorstore",
        message=constants.TESTING_VECTORSTORE,
    )
    logger.info(
    ...
    try:
        ...
        logger.info(
            ...
            message=constants.COLLECTION_INITIALIZED,
        )
...
def _check_environment_variables(logger) -> bool:
    ...
    if not os.getenv(constants.ENV_GEMINI_API_KEY):
        ...
        logger.info(
            codes.ENV_VAR_CHECK,
            help=f"Run: export {constants.ENV_GEMINI_API_KEY}='your-key'",
        )
        return False
...
def check_prerequisites(logger) -> bool:
    ...
    if not all_good:
        logger.error(
            codes.PREREQUISITES_FAILED, message=constants.PREREQUISITES_NOT_MET
        )
        return all_good
...
def _log_setup_success(config: Config, logger) -> None:
    ...
    logger.info(
        codes.SCRIPT_COMPLETED, example=constants.EXAMPLE_DEMO_PROVIDER_SWITCHING
    )
...
def main():
    """Main setup function."""
    logger.info(
        codes.SCRIPT_STARTED, script="setup_database", header=constants.SETUP_HEADER
    )
    ...
    try:
        ...
        logger.error(
            ...
            error=str(e),
            message=constants.FAILED_TO_LOAD_CONFIG,
        )
        return constants.SCRIPT_EXIT_ERROR
    ...
    try:
        ...
        logger.error(
            codes.DIRECTORY_CREATING,
            error=str(e),
            message=constants.FAILED_TO_CREATE_DIRECTORIES,
        )
        return constants.SCRIPT_EXIT_ERROR
...
if __name__ == "__main__":
    try:
        ...
        logger.warning(
            codes.EVENT_APP_SHUTDOWN, script="setup_database", reason="interrupted"
        )
