...
class DocumentLoader:
    ...
    def _log_loading_start(self, path: Path) -> None:
        ...
        """
        logger.info(
            codes.LOADER_LOADING_FILE, file_path=str(path), file_type=path.suffix
        )
    ...
    def _load_with_strategy(self, path: Path) -> List[Document]:
        ...
        try:
            ...
            logger.error(codes.LOADER_LOADING_ERROR, file_path=str(path), error=str(e))
            raise

    def _log_loading_success(self, path: Path, documents: List[Document]) -> None:
        ...
        """
        logger.info(
            codes.LOADER_FILE_LOADED, file_path=str(path), document_count=len(documents)
        )
