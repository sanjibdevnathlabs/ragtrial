...
class FileRepository(BaseRepository[File]):
    ...
    def find_by_filename(
        self, session: Session, filename: str, include_deleted: bool = False
    ) -> Optional[File]:
        """
    ...
    def find_by_checksum(
        self, session: Session, checksum: str, include_deleted: bool = False
    ) -> Optional[File]:
        """
    ...
    def find_unindexed_files(
        ...
        return (
            self.find_by_fields(
                session, filters={"indexed": False}, include_deleted=False
            )[:limit]
            if limit
            else self.find_by_fields(
                session, filters={"indexed": False}, include_deleted=False
            )
        )
    ...
    def find_indexed_files(
        ...
        return (
            self.find_by_fields(
                session, filters={"indexed": True}, include_deleted=False
            )[:limit]
            if limit
            else self.find_by_fields(
                session, filters={"indexed": True}, include_deleted=False
    ...
    def mark_as_indexed(self, session: Session, file_id: str) -> bool:
        """
        ...
        try:
            ...
            if file is None:
                logger.warning(codes.DB_ENTITY_NOT_FOUND, file_id=file_id)
                return False
    ...
    def get_total_size(self, session: Session, include_deleted: bool = False) -> int:
        """
    ...
    def find_by_storage_backend(
        self, session: Session, storage_backend: str, include_deleted: bool = False
    ) -> List[File]:
        """
    ...
    def find_by_file_type(
        self, session: Session, file_type: str, include_deleted: bool = False
    ) -> List[File]:
        """
