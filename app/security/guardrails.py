...
class GuardrailsManager:
    ...
    def validate_input(self, user_query: str) -> Dict[str, Any]:
        ...
        if self.config.enable_input_validation:
            ...
            if not validation_result.is_valid:
                ...
                threat_level = self._max_threat_level(threat_level, validation_result.threat_level)
                
                if self.config.strict_mode:
                    self._log_violation(constants.ERROR_INVALID_INPUT, validation_result, threat_level)
        ...
        if self.config.enable_injection_detection:
            ...
            if not injection_result.is_valid:
                ...
                threat_level = self._max_threat_level(threat_level, injection_result.threat_level)
                
                if self.config.strict_mode:
                    self._log_violation(constants.ERROR_PROMPT_INJECTION, injection_result, threat_level)
        ...
        return {
            ...
            constants.VALIDATION_KEY_SANITIZED_QUERY: sanitized_query if is_safe else "",
            constants.VALIDATION_KEY_VIOLATIONS: violations,
            constants.VALIDATION_KEY_THREAT_LEVEL: threat_level,
            constants.VALIDATION_KEY_ERROR_MESSAGE: constants.ERROR_BLOCKED_BY_GUARDRAILS if not is_safe else ""
        }
    ...
    def validate_output(self, llm_output: str) -> Dict[str, Any]:
        ...
        if self.config.enable_output_validation:
            ...
            if not validation_result.is_valid:
                ...
                if self.config.strict_mode:
                    self._log_violation(constants.ERROR_UNSAFE_OUTPUT, validation_result, threat_level)
        ...
        return {
            ...
            constants.VALIDATION_KEY_VIOLATIONS: violations,
            constants.VALIDATION_KEY_THREAT_LEVEL: threat_level,
            constants.VALIDATION_KEY_ERROR_MESSAGE: constants.ERROR_UNSAFE_OUTPUT if not is_safe else ""
        }
    ...
    def _log_violation(self, error_msg: str, result: ValidationResult, threat_level: str) -> None:
    ...
    def get_security_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            constants.GUARDRAILS_KEY_VALIDATORS: {
                "input_validator": constants.GUARDRAILS_STATUS_ACTIVE if self.config.enable_input_validation else constants.GUARDRAILS_STATUS_DISABLED,
                "injection_detector": constants.GUARDRAILS_STATUS_ACTIVE if self.config.enable_injection_detection else constants.GUARDRAILS_STATUS_DISABLED,
                "output_validator": constants.GUARDRAILS_STATUS_ACTIVE if self.config.enable_output_validation else constants.GUARDRAILS_STATUS_DISABLED,
            }
        }
