...
from app.modules.upload.response import (
    BatchUploadResponse,
    FileUploadResult,
    UploadResponse,
)
...
@router.post(
    "/upload", response_model=Union[UploadResponse, BatchUploadResponse, ErrorResponse]
)
async def upload(
...
async def _handle_single_file_upload(
    file: UploadFile, service: UploadService
) -> UploadResponse:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=ErrorResponse(
                error=str(e), error_code=constants.ERROR_CODE_VALIDATION_ERROR
            ).model_dump(),
        )
...
async def _handle_batch_upload(
    files: List[UploadFile], service: UploadService
) -> BatchUploadResponse:
    """
...
async def _process_single_file_in_batch(
    ...
    try:
        ...
        logger.error(
            codes.API_UPLOAD_FAILED, filename=file.filename, error=str(e), exc_info=True
        )
        return FileUploadResult(
            filename=file.filename,
            success=False,
