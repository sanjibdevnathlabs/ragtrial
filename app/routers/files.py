...
async def list_files(service: FileManagementService = Depends(get_file_service)):
    ...
    try:
        ...
        logger.error(
            codes.API_ERROR, operation="list_files", error=str(e), exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ErrorResponse(
                error=codes.MSG_API_ERROR, error_code="LIST_ERROR"
            ).model_dump(),
        )


@router.get(
    "/files/{filename}", response_model=Union[FileMetadataResponse, ErrorResponse]
)
async def get_file_metadata(
    filename: str, service: FileManagementService = Depends(get_file_service)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=ErrorResponse(
                error=codes.MSG_API_FILE_NOT_FOUND, error_code="FILE_NOT_FOUND"
            ).model_dump(),
        )
    except Exception as e:
        logger.error(
            codes.API_ERROR, operation="get_metadata", error=str(e), exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ErrorResponse(
                error=codes.MSG_API_ERROR, error_code="METADATA_ERROR"
            ).model_dump(),
        )
...
async def delete_file(
    ...
    try:
        ...
        logger.error(
            codes.API_ERROR, operation="delete_file", error=str(e), exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ErrorResponse(
                error=codes.MSG_API_ERROR, error_code="DELETE_ERROR"
            ).model_dump(),
        )
