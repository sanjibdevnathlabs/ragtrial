...
def _demonstrate_filtered_query(vectorstore):
    ...
    logger.info(
        codes.VECTORSTORE_QUERYING, query=query, filter=f"topic = '{filter_topic}'"
    )

    results = vectorstore.query(query, n_results=2, where={"topic": filter_topic})
...
def main():
    ...
    vectorstore.add_documents(
        texts=_get_sample_documents(), metadatas=_get_sample_metadata()
    )
    ...
    logger.info(
        codes.DEMO_INSTRUCTIONS, step="2. Change [vectorstore] provider = 'pinecone'"
    )
    logger.info(
        codes.DEMO_INSTRUCTIONS, step="3. Run this script again - NO CODE CHANGES!"
    )
